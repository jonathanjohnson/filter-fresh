  ...
  return (
    <section className="container py-14 md:py-20">
      <div className="mx-auto max-w-4xl">
        ...
        <p className="mt-4 max-w-2xl text-ff-ink-2">
          ...
          {posts.map((p) => (
            <li key={p.slug} className="flex flex-col rounded-lg border bg-card p-6">
              ...
              <h2 className="mt-2 text-xl font-semibold">
                <Link href={`/blog/${p.slug}`} className="hover:text-ff-brand">
                  {p.title}
                </Link>
              </h2>
              <p className="mt-2 flex-1 text-sm text-ff-ink-2">{p.description}</p>
              <div className="mt-4 flex items-center justify-between text-xs text-ff-ink-2">
